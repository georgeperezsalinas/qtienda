...
| Google Search Console | Verificacion del sitio (`googleae51a2aad375bb03.html` en `frontend/public/`, no en la raiz del repo como se intento primero — Next.js solo sirve estaticos desde `public/`) y sitemap dinamico (`/sitemap.xml`, ya existia via `sitemap.ts`, incluye landing + `/tiendas` + cada tienda activa) registrados en Search Console. | qtienda.shop indexable y monitoreado por Google. |
| SEO/OG de tiendas | Imagen OG generada dinamicamente por tienda (`opengraph-image.tsx`: banner real > logo real > iniciales con su color de marca) — antes el fallback era un SVG que WhatsApp/Facebook/LinkedIn no renderizan como og:image, asi que compartir el link de una tienda sin foto no mostraba ninguna preview. Ademas: `og:title`/`twitter:title` ahora usan el `meta_title` configurado por el vendedor (antes se ignoraba fuera de la pestaña); JSON-LD con pais, moneda y metodos de pago reales de cada tienda en vez de asumir siempre Peru/soles/"Cash, Credit Card, Mobile Payment" para todas. | Compartir una tienda por WhatsApp (el canal principal de distribucion del producto) ahora siempre muestra una tarjeta con imagen, y los datos estructurados dejan de ser falsos para tiendas fuera de Peru. |
| Banner por defecto | `BannerPlaceholder` en la tienda publica: degradado con el `primary_color` real de la tienda + logo/iniciales + nombre, cuando el vendedor no subio ningun banner (antes esa seccion se saltaba por completo, dejando un salto visual del header directo a productos). Mismo criterio que el fallback de logo (siempre calculado en vivo, nunca un archivo generado). | Toda tienda se ve terminada desde el primer momento, suba o no banner. |
...
| Tienda publica | Funcional | 87% | `frontend/src/app/tienda/[slug]/page.tsx`, `components/store/StorePage.tsx` | SEO, JSON-LD (con pais/moneda/pago reales por tienda 2026-07-22), imagen OG dinamica por tienda, badge "personas viendo esto", banner por defecto con el color real cuando no suben uno, busqueda, categorias, carrito, PWA, tour guiado, seguimiento de pedido, cross-sell, countdown de oferta, banner de cupon de bienvenida. | Analytics por tienda mas fino; revisar overlap de badges en `ProductDetailSheet` (bug preexistente menor, no bloqueante). |
...
| QT-049 | P1 | SEO | Google Search Console verificado (`googleae51a2aad375bb03.html` movido a `frontend/public/`, unica carpeta que Next.js sirve en la raiz del dominio) y sitemap dinamico registrado (`/sitemap.xml`, ya existia, incluye cada tienda activa). | qtienda.shop indexable y monitoreado por Google. | Hecho 2026-07-22 |
| QT-050 | P1 | SEO/Tiendas | Imagen OG generada por tienda (`opengraph-image.tsx`: banner real > logo real > iniciales) — el fallback SVG anterior no se veia en WhatsApp/Facebook/LinkedIn al compartir. Titulo SEO del vendedor (`meta_title`) ahora tambien se ve al compartir (antes solo en la pestaña). JSON-LD con pais/moneda/metodos de pago reales de cada tienda en vez de asumir siempre Peru/soles/pagos genericos. | Compartir por WhatsApp (canal principal del producto) siempre muestra imagen; datos estructurados honestos para tiendas fuera de Peru. | Hecho 2026-07-22 |
| QT-051 | P2 | Tienda publica | Banner por defecto (`BannerPlaceholder`): degradado con el color real de la tienda + logo/iniciales + nombre cuando no subieron banner (antes esa seccion desaparecia por completo). | Toda tienda se ve terminada desde el primer momento. | Hecho 2026-07-22 |
