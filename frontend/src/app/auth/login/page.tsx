...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Guardar tokens ANTES de llamar /me para que el interceptor use el nuevo token
      setTokens(data.access_token, data.refresh_token);

      // Llamar /me con axios directo (sin interceptor) para evitar conflictos
  ...
  return (
    <div className="min-h-dvh flex" style={{ background: "var(--surface-2)" }}>
      ...
      <div
        ...
        style={{ background: "linear-gradient(160deg, var(--brand-800) 0%, #4C1D95 100%)" }}
      >
        <Link href="/" className="font-display font-extrabold text-2xl text-white">
          q<span style={{ color: "rgba(255,255,255,.5)" }}>tienda</span>
        </Link>
          ...
          <ul className="space-y-3">
            {["Tu tienda siempre activa", "Pedidos en tiempo real", "Sin comisiones"].map((f) => (
              <li key={f} className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium" style={{ color: "rgba(255,255,255,.8)" }}>{f}</span>
              </li>
            ))}
          </ul>
        </div>

        <p className="text-xs" style={{ color: "rgba(255,255,255,.35)" }}>© 2025 qtienda.shop</p>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-5 py-10">
        ...
        <Link href="/" className="lg:hidden mb-8">
          <span className="font-display font-extrabold text-2xl" style={{ color: "var(--brand-600)" }}>
        ...
        >
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <div>
              ...
              <input id="email" className="input" type="email" inputMode="email"
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button type="button" className="text-xs font-semibold" style={{ color: "var(--brand-600)" }}>
                  ¿La olvidaste?
                </button>
              </div>
              <div className="relative">
                <input id="password" className="input pr-12"
                  type={showPass ? "text" : "password"}
                  placeholder="Tu contraseña" autoComplete="current-password"
                  value={password} onChange={(e) => setPassword(e.target.value)} />
                <button type="button" onClick={() => setShowPass((v) => !v)}
                  className="absolute right-3.5 top-1/2 -translate-y-1/2"
                  style={{ color: "var(--ink-3)" }} aria-label="Mostrar contraseña">
          ...
          {/* Footer: accesos rápidos según rol */}
          <div
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <p className="text-[10px] mt-0.5" style={{ color: "var(--brand-500)" }}>
                  Soy vendedor
                </p>
              </div>
            </Link>
